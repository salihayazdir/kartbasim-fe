...
type ConsumableActionsDropdownProps = {
  record: Consumable;
  setEditConsumableModalIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
  setDeleteConsumableModalIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
  setSelectedRecord: React.Dispatch<React.SetStateAction<Consumable | null>>;
};

export default function ConsumableActionsDropdown({
  record,
  setEditConsumableModalIsOpen,
  setDeleteConsumableModalIsOpen,
  setSelectedRecord,
}: ConsumableActionsDropdownProps) {
  return (
    <Menu as={'div'} className=''>
      ...
      <Menu.Items className='absolute z-10 flex -translate-x-[calc(100%-50px)] flex-col gap-1 rounded-lg border border-slate-200 bg-white p-2 text-xs shadow-lg'>
        ...
        <Menu.Item>
          <button
            onClick={() => {
              ...
              setEditConsumableModalIsOpen(true);
            }}
        ...
        <Menu.Item>
          <button
            onClick={() => {
              setSelectedRecord(record);
              setDeleteConsumableModalIsOpen(true);
            }}
