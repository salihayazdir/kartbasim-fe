...
type ProductGroupActionsDropdownProps = {
  record: ProductGroup;
  setEditProductGroupModalIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
  setDeleteProductGroupModalIsOpen: React.Dispatch<
    React.SetStateAction<boolean>
  >;
  setSelectedRecord: React.Dispatch<React.SetStateAction<ProductGroup | null>>;
};

export default function ProductGroupActionsDropdown({
  record,
  setEditProductGroupModalIsOpen,
  setDeleteProductGroupModalIsOpen,
  setSelectedRecord,
}: ProductGroupActionsDropdownProps) {
  return (
      ...
      <Menu.Items className='absolute z-10 flex -translate-x-[calc(100%-50px)] flex-col gap-1 rounded-lg border border-slate-200 bg-white p-2 text-xs shadow-lg'>
        ...
        <Menu.Item>
          <button
            onClick={() => {
              ...
              setEditProductGroupModalIsOpen(true);
            }}
        ...
        <Menu.Item>
          <button
            onClick={() => {
              setSelectedRecord(record);
              setDeleteProductGroupModalIsOpen(true);
            }}
