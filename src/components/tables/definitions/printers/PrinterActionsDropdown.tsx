...
type PrinterActionsDropdownProps = {
  record: Printer;
  setEditPrinterModalIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
  setDeletePrinterModalIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
  setSelectedRecord: React.Dispatch<React.SetStateAction<Printer | null>>;
};

export default function PrinterActionsDropdown({
  record,
  setEditPrinterModalIsOpen,
  setDeletePrinterModalIsOpen,
  setSelectedRecord,
}: PrinterActionsDropdownProps) {
  return (
      ...
      <Menu.Items className='absolute z-10 flex -translate-x-[calc(100%-50px)] flex-col gap-1 rounded-lg border border-slate-200 bg-white p-2 text-xs shadow-lg'>
        ...
        <Menu.Item>
          <button
            onClick={() => {
              ...
              setEditPrinterModalIsOpen(true);
            }}
        ...
        <Menu.Item>
          <button
            onClick={() => {
              setSelectedRecord(record);
              setDeletePrinterModalIsOpen(true);
            }}
