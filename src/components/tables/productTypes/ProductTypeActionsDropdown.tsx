...
type ProductTypeActionsDropdownProps = {
  record: ProductType;
  setEditProductTypeModalIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
  setDeleteProductTypeModalIsOpen: React.Dispatch<
    React.SetStateAction<boolean>
  >;
  setSelectedRecord: React.Dispatch<React.SetStateAction<ProductType | null>>;
};

export default function ProductTypeActionsDropdown({
  record,
  setEditProductTypeModalIsOpen,
  setDeleteProductTypeModalIsOpen,
  setSelectedRecord,
}: ProductTypeActionsDropdownProps) {
  return (
      ...
      <Menu.Items className='absolute z-10 flex -translate-x-[calc(100%-50px)] flex-col gap-1 rounded-lg border border-slate-200 bg-white p-2 text-xs shadow-lg'>
        ...
        <Menu.Item>
          <button
            onClick={() => {
              ...
              setEditProductTypeModalIsOpen(true);
            }}
        ...
        <Menu.Item>
          <button
            onClick={() => {
              setSelectedRecord(record);
              setDeleteProductTypeModalIsOpen(true);
            }}
